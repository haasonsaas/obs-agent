...
from .exceptions import (
    OBSAgentError, SceneNotFoundError, SourceNotFoundError,
    StreamAlreadyActiveError, StreamNotActiveError,
    RecordingAlreadyActiveError, RecordingNotActiveError,
    ValidationError
)
from .validation import (
    validate_scene_name, validate_source_name, validate_filter_name,
    validate_volume, validate_file_path, validate_settings,
    validate_transition_duration, validate_resolution
)
from .logging import (
    get_logger, setup_logging, log_performance,
    log_obs_request, log_obs_response, log_obs_error,
    log_scene_change, log_stream_status, log_recording_status
)
...
class OBSAgent:
    ...
    async def connect(self) -> bool:
        ...
        self.logger.info(
            ...
            f"(WebSocket {version_info['websocket_version']})"
        )
    ...
    async def toggle_source_mute(self, source_name: str) -> bool:
        ...
        response = await self.connection.execute(
            requests.ToggleInputMute(inputName=source_name)
        )
    ...
    async def set_transition(self, transition_name: str) -> bool:
        ...
        """
        await self.connection.execute(
            requests.SetCurrentSceneTransition(transitionName=transition_name)
        )
    ...
    async def set_transition_duration(self, duration_ms: int) -> bool:
        ...
        await self.connection.execute(
            requests.SetCurrentSceneTransitionDuration(transitionDuration=duration_ms)
        )
