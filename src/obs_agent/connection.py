...
from .exceptions import (
    ConnectionError, AuthenticationError, OBSAgentError,
    handle_obs_error, RequestTimeoutError
)
...
class ConnectionManager:
    ...
    async def execute(self, request: Any, timeout: Optional[float] = None) -> Any:
        ...
        try:
            ...
            if timeout:
                ...
                response = await asyncio.wait_for(
                    asyncio.get_event_loop().run_in_executor(
                        None, self._connection.call, request
                    ),
                    timeout=timeout
                )
            else:
            ...
            if elapsed > 1.0:  # Log slow requests
                logger.warning(
                    f"Slow request: {type(request).__name__} took {elapsed:.2f}s"
                )
    ...
    def _handle_connection_error(self, error: Exception) -> None:
        ...
        if "auth" in error_str or "password" in error_str:
            raise AuthenticationError(
                "Authentication failed. Check your OBS WebSocket password."
            )
            ...
            raise ConnectionError(
                f"Connection timeout to OBS at {self._config.host}:{self._config.port}"
            )
        else:
    ...
    async def _auto_reconnect(self) -> None:
        ...
        for attempt in range(1, self._config.max_reconnect_attempts + 1):
            ...
            logger.info(
                f"Reconnection attempt {attempt}/{self._config.max_reconnect_attempts}"
            )
