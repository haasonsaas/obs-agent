...
from .domain import (
    ...
    AutomationRuleTriggered, AutomationRuleExecuted
)
...
class EventSourcingSystem:
            ...
            if original_trigger:
                async def trigger_interceptor(rule, trigger_data):
                    ...
                    triggered_event = AutomationRuleTriggered(
                        ...
                        metadata=EventMetadata(
                            correlation_id=uuid4()  # New correlation for each trigger
                        ),
                        rule_id=rule.id,
                        rule_name=rule.name,
    ...
    async def start(self) -> None:
        ...
        if self.projection_builder and self.config.enable_projections:
            await self.projection_builder.start_continuous_update(
                self.config.projection_update_interval
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if self.projection_builder:
            stats["projections"] = {
                ...
                for name, projection in self.projection_builder.projections.items()
            }
    ...
    def start_debugging(self, hours_back: int = 24) -> DebugSession:
        ...
        return self.debugger.start_session(
            start_time=datetime.utcnow() - timedelta(hours=hours_back)
        )
    
    def replay_automation(self, 
                         rule_id: str,
        ...
        for event in events:
            if event.event_type == EventType.AUTOMATION_RULE_TRIGGERED:
                ...
                correlation_events = self.event_store.get_correlation_chain(
