...
from typing import (
    Any, Callable, Dict, List, Optional, Set, Tuple, Type, AsyncIterator
)
...
class EventStore:
    ...
    def get_events_by_type(self, event_type: EventType,
                          ...
                          until: Optional[datetime] = None) -> List[DomainEvent]:
    ...
    def replay_events(self, 
                     ...
                     event_types: Optional[Set[EventType]] = None) -> List[DomainEvent]:
        """
    ...
    async def stream_events(self, 
                           ...
                           batch_size: int = 100) -> AsyncIterator[List[DomainEvent]]:
        """
    ...
    def get_correlation_chain(self, correlation_id: UUID) -> List[DomainEvent]:
        ...
        with self._lock:
            return [
                e for e in self._global_event_stream
            ...
            for aggregate_id in list(self._events_by_aggregate.keys()):
                ...
                if keep_snapshots:
                    ...
                    if snapshot and snapshot.timestamp < before:
                        ...
                        filtered = [
                            e for e in events 
                            if e.metadata.version > snapshot.version
                        ]
                    ...
                    filtered = [
                        e for e in events 
                        if e.metadata.timestamp >= before
                    ]
                    removed += len(events) - len(filtered)
                    self._events_by_aggregate[aggregate_id] = filtered
