...
class TimeTravelDebugger:
    ...
    def start_session(self, 
                     ...
                     end_time: Optional[datetime] = None) -> DebugSession:
    ...
    def goto(self, timestamp: datetime) -> Dict[str, Any]:
        ...
        events = [
            e for e in self.current_session.events_in_range
    ...
    def step_forward(self, count: int = 1) -> Tuple[List[DomainEvent], Dict[str, Any]]:
        ...
        future_events = [
            e for e in self.current_session.events_in_range
            if e.metadata.timestamp > current_pos
        ]
    ...
    def what_if(self, 
               modify_events: Callable[[List[DomainEvent]], List[DomainEvent]],
               at_timestamp: Optional[datetime] = None) -> Dict[str, Any]:
        """
    ...
    def find_event_pattern(self, 
                          pattern: List[EventType],
                          within: Optional[timedelta] = None) -> List[List[DomainEvent]]:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_active_aggregates": sorted(
                aggregate_counts.items(), 
                key=lambda x: x[1], 
                reverse=True
            )[:10],
            "event_rate_per_second": event_rate,
            "unique_correlations": len(set(
                e.metadata.correlation_id for e in events 
                if e.metadata.correlation_id
            )),
            "time_span": str(
                events[-1].metadata.timestamp - events[0].metadata.timestamp
            ) if events else "0:00:00"
        }
