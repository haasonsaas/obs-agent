...
def sanitize_string(value: str, max_length: int = MAX_NAME_LENGTH, 
                   allow_spaces: bool = True, pattern: Optional[re.Pattern] = None) -> str:
    """
    ...
    if isinstance(color, str):
        ...
        if not COLOR_HEX_PATTERN.match(color):
            raise ValidationError(
                ...
                "Use hex format like #RRGGBB or RRGGBB"
            )
...
def validate_volume(volume: Union[float, int], as_db: bool = False) -> float:
    ...
    if as_db:
        ...
        if not 0.0 <= volume_float <= 20.0:
            raise ValidationError(
                f"Volume multiplier must be between 0.0 and 20.0, got {volume_float}"
            )
...
def validate_settings(settings: Dict[str, Any], allowed_keys: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    if allowed_keys:
        ...
        if invalid_keys:
            raise ValidationError(
                f"Settings contain invalid keys: {', '.join(invalid_keys)}. "
