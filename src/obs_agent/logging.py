...
def log_obs_request(request_type: str, **params):
    ...
    logger.debug(
        f"OBS Request: {request_type}",
        extra={
            'obs_request': request_type,
            'params': params
        }
    )
...
def log_recording_status(is_recording: bool, file_path: Optional[str] = None):
    ...
    if is_recording:
        logger.info(
            "Recording started",
            extra={'event': 'recording_start'}
        )
    else:
        logger.info(
            f"Recording stopped: {file_path}",
            extra={
