...
class OBSConnection:
    ...
    def get_agent(self) -> AdvancedOBSAgent:
        if self._obs_agent is None:
            self._obs_agent = AdvancedOBSAgent(host="localhost", port=4455, password="")  # Will be set from env
...
class OBSAudioTool(BaseTool):
    ...
    def _run(self, source_name: str, volume_db: Optional[float] = None, muted: Optional[bool] = None) -> str:
        """Synchronous wrapper for audio operations"""
        return asyncio.run(self._arun(source_name, volume_db, muted))

    async def _arun(self, source_name: str, volume_db: Optional[float] = None, muted: Optional[bool] = None) -> str:
...
class OBSStatsTool(BaseTool):
    ...
    async def _arun(self) -> str:
        ...
        try:
            ...
            return (
                f"CPU: {cpu:.1f}%, Memory: {memory/1024/1024:.0f}MB, "
                f"FPS: {fps:.1f}, "
...
class OBSFilterTool(BaseTool):
    ...
    description: str = (
        "Manage filters on sources - add, remove, or configure filters like chroma key, noise suppression"
    )
    ...
    async def _arun(
        ...
        try:
            ...
            # Add custom filter
                ...
                success = await obs.add_filter(
                    source_name=source_name, filter_name=filter_name, filter_kind=filter_type, filter_settings=settings
                )
...
class OBSSnapshotTool(BaseTool):
    ...
    async def _arun(self) -> str:
        ...
        try:
            ...
            return (
                f"Scene: {snapshot['current_scene']} ({snapshot['available_scenes']} total), "
