        ...
        async with create_obs_agent() as agent:
            ...
            for source in sources:
                if source["inputKind"] in ["wasapi_input_capture", "wasapi_output_capture"]:
                    ...
                    logger.info(f"Audio source '{source['inputName']}': " f"{volume['volume_db']:.1f} dB")
        ...
        if status["is_streaming"]:
            ...
            for i in range(10):
                ...
                if status["total_frames"] > 0:
                    ...
                    logger.info(f"Streaming [{i+1}/10s]: " f"Dropped frames: {drop_rate:.1f}%")
    ...
    async with create_obs_agent() as agent:
        ...
        with log_context(action="recording", user="demo"):
            ...
            for i in range(5):
                ...
                logger.info(f"Recording [{i+1}/5s]: " f"{status['bytes'] / 1024 / 1024:.1f} MB")
...
async def screenshot_example():
    ...
    async with create_obs_agent() as agent:
        ...
        if video_source:
            ...
            await agent.take_screenshot(video_source, screenshot_path, width=1280, height=720)  # Resize to 720p
