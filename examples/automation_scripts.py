...
async def gaming_stream_automation():
    async with OBSAutomation() as automation:
        ...
        await automation.controller.create_pip_layout(
            main_source="Game Capture",
            pip_source="Webcam",
            pip_position=(0.75, 0.75),
            pip_scale=0.2
        )
        ...
        monitor_task = asyncio.create_task(
            automation.controller.monitor_stream_health(health_alert)
        )
...
async def tutorial_recording_automation():
    async with OBSAutomation() as automation:
        ...
        sections = [
            {
                "title": "Introduction",
                "instruction": "Welcome to the tutorial",
                "duration": 10
            },
            {
                "title": "Setup",
                "instruction": "First, let's configure the settings",
                "duration": 30
            },
            {
                "title": "Main Content",
                "instruction": "Now for the main demonstration",
                "duration": 120
            },
            {
                "title": "Conclusion",
                "instruction": "Thank you for watching!",
                "duration": 10
            }
        ]
        ...
        for section in sections:
            ...
            await automation.agent.set_source_settings(
                "Instructions",
...
async def dynamic_overlay_automation():
    async with OBSAutomation() as automation:
        ...
        overlays = {
            "follower_alert": {
                "text": "New Follower: {name}",
                "duration": 5,
                "position": (0.5, 0.1)
            },
            "donation_alert": {
                "text": "{name} donated ${amount}",
                "duration": 10,
                "position": (0.5, 0.9)
            },
            "chat_highlight": {
        ...
        for alert in alerts:
            ...
            for item in scene_items:
                if item.get("sourceName") == alert_type:
                    await automation.agent.set_scene_item_enabled(
                        "Main", item["sceneItemId"], True
                    )
                    break
