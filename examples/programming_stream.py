...
from src.tools import OBSAgentOrchestrator, OBSAIAgent, StreamGoal
...
class ProgrammingStreamSetup:
    ...
    async def _create_programming_scenes(self):
        ...
        scenes = {
            ...
            "📄 Documentation": {"description": "Showing docs or references", "sources": ["Browser Capture", "Notes"]},
            "🔀 Split View": {
                "description": "Code + Terminal side by side",
                "sources": ["Screen Capture", "Terminal Window"],
            },
            "🎯 Focus Mode": {"description": "Zoomed code section", "sources": ["Cropped Screen", "Line Highlight"]},
            "💭 Explaining": {
            ...
            "☕ Break": {"description": "Break screen", "sources": ["Break Message", "Music Visualizer"]},
            "📺 Starting Soon": {"description": "Pre-stream screen", "sources": ["Starting Text", "Schedule"]},
            "🎉 Ending": {"description": "End stream screen", "sources": ["Thanks Message", "Social Links"]},
        }
        ...
        if include_webcam:
            try:
                await self.agent.create_source(
                    ...
                    settings={"device": 0, "preset": "1280x720"},
                )
                ...
                for item in scene_items:
                    if item.get("sourceName") == "Webcam":
                        ...
                        await self.agent.set_scene_item_transform("💻 Code Editor", item["sceneItemId"], transform)
        ...
        for overlay in overlays:
            ...
            try:
                await self.agent.create_source(
                    ...
                    settings={
                        "text": overlay["text"],
                        "font": {"face": "Arial", "size": overlay["size"], "style": "Bold"},
            ...
            if theme == "dark":
                ...
                try:
                    await self.agent.add_filter(
                        ...
                        filter_settings={"brightness": 0.05, "contrast": 0.02, "saturation": 1.1},
                    )
        ...
        try:
            await self.agent.create_source(
                ...
                settings={
                    ...
                    "font": {"face": "Consolas", "size": 18, "style": "Regular"},
                    "color1": 0xFFFFFFFF,
                    "color2": 0xFFFFFFFF,
                    "align": "left",
                    "valign": "bottom",
            ...
            for item in scene_items:
                if item.get("sourceName") == "Shortcuts Display":
                    transform = {"positionX": 10, "positionY": 1080 - 50, "scaleX": 1.0, "scaleY": 1.0}
                    await self.agent.set_scene_item_transform("💻 Code Editor", item["sceneItemId"], transform)
    ...
    async def quick_setup_and_test(self):
        ...
        try:
            ...
            await self.create_programming_environment(
                language="python", theme="dark", include_webcam=True, terminal_position="bottom"
            )
